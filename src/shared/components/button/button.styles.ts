...
const generateButtonNonOutStyles = (styles: ButtonNonOutStyles): string => {
	const { color, hoverColor, focusColor, disabledColor } = styles
...
const generateButtonStyles = (styles: ButtonStyles): string => {
	...
	return css`
		...
		box-shadow:
			4px 4px 20px rgba(35, 162, 0, 0.25),
			-4px -4px 20px rgba(35, 162, 0, 0.25);
		...
		&:disabled {
			...
			${disabledBorderColor ? `border: 1px solid var(--${disabledBorderColor});` : `border: 1px solid transparent;`}
			${disabledColor
				? `
			color: var(--${disabledColor});
			& path {
